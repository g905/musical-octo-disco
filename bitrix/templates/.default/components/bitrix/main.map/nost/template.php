...
?>
		...
		<?foreach($arResultProduction as $index => $arItem1):?>
					<?   // adl 18.04.14 Это для раскрутки запросов "световые короба на автомобили"
						if ($arItem1["NAME"] == "Световые рекламные короба") $arItem1["NAME"] = "Световые короба на такси";
					?>
		...
		<?endforeach?>
				...
				<div class="col-xs-9 text-center" id="foot3">
					<h3 class="h3"><a href="/information/">Информация</a></h3>
					<div id="links">
						<ul class="media-list text-left ulmenu">
		<?foreach($arResultInformation as $index => $arItem2):?>
					<? // adl 17.04.14 Это для раскрутки запросов "оборудование для такси"
						if ($arItem2["FULL_PATH"] == "/information/32/") $arItem2["FULL_PATH"] = "/information/32/138/";
					?>
