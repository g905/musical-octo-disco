...
<div id="<?=$block_id;?>" class="<?=$arResult['SLIDER_CSS_CLASS']?>" style="<?if($arParams['WIDTH']):?>width:<?=$arParams['WIDTH'];?>;<?endif;?><?if($arParams['HEIGHT']):?>height:<?=$arParams['HEIGHT'];?><?endif;?>left: -59px;">
	...
	<div class="beono-banner_slider-pager">
		...
		<?foreach($arResult['BANNERS'] as $key=>$arBanner):?>
			...
			<div <?if($key==0):?>class="activ"<?endif;?>>
				<a <?if($key==0):?>class="active"<?endif;?> href="#" title="<?=$arBanner['FIELDS']['NAME'];?>" style="text-decoration: none;"><?=$page_value?></a>
			</div>
		<?endforeach;?>
		</div>
	</div>
