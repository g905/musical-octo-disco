...
var JotForm = {
    ...
    texts: {
        ...
        incompleteFields:   'Есть незаполненные обязательные поля. Пожалуйста, заполните их.',
        required:           'Обязательное поле.',
        requireOne:         'По крайней мере одно поле должно быть заполнено.', 
        ...
        inputCarretErrorA:  'Введенное значение не может быть меньше:',
        ...
        generalError:       'Есть ошибки в заполнении полей. Пожалуйста, исправьте их перед отправкой формы.',
    ...
    */
    ...
    description: function(input, message){
        ...
        if((cont.getWidth()/2) < bubble.getWidth()){
            bubble.setStyle('right: -' + ( cont.getWidth() - ( right ? 100 : 20 ) -150 ) + 'px');
        }
    ...
    validator: function(){
        ...
        var reg = {
            ...
            alphanumeric: /^[a-zA-ZГ¦ГёГҐГ¤Г¶Г†ГГ…Г„Г–0-9\s]+$/,
            numeric: /^(-?\d+[\.]?)+$/,
            numericDotStart: /^([\.]\d+)+$/,  //accept numbers starting with dot
            alphabetic: /^[a-zA-ZГ¦ГёГҐГ¤Г¶Г†ГГ…Г„Г–\s]+$/,
